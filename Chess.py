...
class Board:
    def __init__(self) -> None:
        ...
        self.canvas = tkinter.Canvas(self.root, width=self.canWidth, 
                                     height=self.canHeight)
        ...
        self.ligtWhite = self.changeCol(*tuple(int(c * 0.4 + 255 * 0.6) 
                                               for c in (102, 216, 185)))
        self.ligtBlack = self.changeCol(*tuple(int(c * 0.4 + 255 * 0.6) 
                                               for c in (5, 255, 5)))

    def initBoard(self) -> list[list[Piece]]: 
        return [[Rook("b"), Knight("b"), Bishop("b"), Queen("b"), King("b"), 
                 Bishop("b"), Knight("b"), Rook("b")],
                ...
                [Rook('w'), Knight("w"), Bishop("w"), Queen("w"), King("w"), 
                 Bishop("w"), Knight("w"), Rook("w")]]
    ...
    def handleSqr(self, row, col, x, y):
        if self.boardState.isPicking:
            ...
            if (self.board[row][col] is None or
                piece.color != self.boardState.player):
                return
            ...
            if (row, col) in validSqr:
                ...
                # move piece
                oldPiece.makeMove(oldRow, oldCol, row, col, self.size, x, y, 
                                  self.boardState.piecesTaken, self.board, 
                                  self.canvas)
                ...
                oldPiece.pawnOnOppositeSide(self.boardState.gameOver[0], 
                                            self.board, promotion)
                #change player
    ...
    def handle_click(self, event):
        if not self.boardState.gameOver[0]:
            col = event.x
            row = event.y
    ...
    def introImage(self, drawBoard):
        begin = Image.open('./img/LetsBegin.png')
        begin = begin.resize((6 * self.size, 6 * self.size))
        self.begin = ImageTk.PhotoImage(begin)
        self.canvas.create_image(self.canWidth // 2, self.canHeight // 2, 
                                 image=self.begin, tags="beginImg")
        def f():
    ...
    def initDraw(self):
        #border
        self.canvas.create_rectangle(self.label, self.label, 
                                     8 * self.size + self.label * 3, 
                                     8 * self.size + self.label * 3)
        # square colors 
        ...
        for row in range(8):
            if row % 2 == 0:
                ...
                color = self.black
            for col in range(8):
                self.canvas.create_rectangle(col * self.size + self.offset, 
                                             row * self.size + self.offset, 
                                             col * self.size + self.size + 
                                             self.offset, 
        ...
        for col in range(8):
            # letters up and down 
            self.canvas.create_text(col * (self.size) + self.offset + 
                                    self.size / 2, 
                                    self.offset - self.label / 2, 
                                    text=letters[col])
            self.canvas.create_text(col * (self.size) + self.offset + 
                                    self.size / 2, 
                                    self.offset + self.label/2 + (self.size * 8), 
                                    text=letters[col])
            # numbers left and right
            self.canvas.create_text(self.offset - self.label / 2, 
                                    (col * self.size)+self.size / 2 + self.offset, 
                                    text=nums[col])
            self.canvas.create_text(self.offset + self.size * 8 + self.label / 2, 
                                    (col * self.size) + self.size / 2 + self.offset, 
                                    text=nums[col])
        ...
        for row in range(8):
            for col in range(8):
                self.boardPos.append((col * self.size + self.offset, 
                                      row * self.size + self.offset, 
                                      col * self.size + self.size + self.offset, 
                                      row * self.size + self.size + self.offset, 
                                      (row, col)))
                obj = self.board[row][col]
                if obj != None:
                    self.pieces.append(obj.tkImage(self.size - self.size * 0.1))
                    pImg = self.canvas.create_image(self.offset + (col*self.size) +
                                                    self.size/2, 
                                                    self.offset + (row*self.size) + 
                                                    self.size/2, 
                                                    image=self.pieces[-1])
        ...
        # Stats
        self.boardState.drawStats(self.size * 8 + (4 * self.label), self.label, 
                                  self.size * 8 + (3 * self.label) + self.size * 5, 
                                  self.size * 8 + (3 * self.label), self.size)
        self.boardState.setTurn()
